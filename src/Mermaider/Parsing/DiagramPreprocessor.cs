...
internal static partial class DiagramPreprocessor
{
	...
	private static (string CleanedText, string? Title) StripFrontmatter(string text)
	{
		...
		// Parse the one supported frontmatter key from simple YAML (no full YAML parser
		// dependency). Every other key is ignored and removed with the frontmatter block;
		// unsupported configuration therefore cannot reach a parser or rendering sink.
		string? title = null;
