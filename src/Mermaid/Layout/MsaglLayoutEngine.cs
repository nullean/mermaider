...
internal static class MsaglLayoutEngine
{
	...
	private static void ConfigureDirection(SugiyamaLayoutSettings settings, Models.Direction direction)
	{
		...
		{
			Models.Direction.LR => new PlaneTransformation(0, -1, 0, 1, 0, 0),
			Models.Direction.RL => new PlaneTransformation(0, 1, 0, -1, 0, 0),
			Models.Direction.BT => new PlaneTransformation(-1, 0, 0, 0, -1, 0),
		...
		StrictModeOptions? strict = null)
	{
		...
		foreach (var (msaglEdge, mermaidEdge) in edgeMap)
		{
			...
			Models.Point? labelPos = null;
			if (msaglEdge.Label != null)
			{
				var lc = msaglEdge.Label.Center;
				labelPos = new Models.Point(lc.X + offsetX, lc.Y + offsetY);
			}
