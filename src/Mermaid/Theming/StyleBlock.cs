...
internal static class StyleBlock
{
	...
	internal static void AppendStyleBlock(StringBuilder sb, string font, bool hasMonoFont)
	{
		...
		if (hasMonoFont)
		{
			sb.Append("  @import url('https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500&amp;display=swap');\n");
		}

		sb.Append("  text { font-family: '").Append(font).Append("', system-ui, sans-serif; }\n");

		if (hasMonoFont)
			sb.Append("  .mono { font-family: 'JetBrains Mono', 'SF Mono', 'Fira Code', ui-monospace, monospace; }\n");
